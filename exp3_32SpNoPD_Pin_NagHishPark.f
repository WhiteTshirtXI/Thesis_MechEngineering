...
c       EPSY=1.049
       EPSY=1.13
      DY=DELTA*(EPSY-1.)/(EPSY**(NYCLM1-1)-1.)
      ETA(1)=0.0
      DO 5 J=2,NYCL
      ETA(J)=ETA(J-1)+DY
    5 DY=EPSY*DY
...
c    even though some are not yet necessary. It just helps debugging. 
      ...
      TAUW=DENSIT*UREF**2.*0.0225*(1./RE)**0.25	!Newtonian expression

      TAUWB=TAUW
      TAUWT=TAUW
      UT=DSQRT(TAUW/DENSIT)		!friction velocity
...
c       TAUW=DPDX(2)*DIA/(2.**INGEOM)		!DPDX is constant so any point is OK
       IF (ISTEP .LE. 1000) THEN
         TAUWB=VISCOS*DUDY(1)
	 TAUWT=VISCOS*DUDY(NY)
       ELSE
         TAUWB=XMULM(1)*DUDY(1)
	 TAUWT=XMULM(NY)*DUDY(NY)
       END IF
C       TAUWB=TAUW
C       TAUWT=TAUW
...
C   End of calculation of elastic stress and other viscoelastic quantities
 ...
 124  CONTINUE
      ...
      TKS(1)=0.D0			!FTP- perhaps irrelevant
      TKS(NY)=0.D0		!FTP- perhaps irrelevant
      XMUT(1)=0.D0		!FTP- perhaps irrelevant
      XMUT(NY)=0.D0		!FTP- perhaps irrelevant
      XMUL(1)=VISS		!FTP- IMPORTANT
      XMUL(NY)=VISS		!FTP- IMPORTANT
      UV(1)=0.D0			!FTP- perhaps irrelevant
      UV(NY)=0.D0			!FTP- perhaps irrelevant
...
c
c     subrelaxação. Se lambda = 0.1 é conveniente usar subrela com coeficientes 0.1 e 0.9
c     se lambda da ordem de 1 são necessários coeficientes de subrelaxação de 0.05 e 0.95
c     se lambda da ordem de 10 são necessários coeficientes de subrelaxação de 0.01 e 0.99
...
C     BOUNDARY CONDITIONS 
      ...
      IF(MODEL.EQ.1) THEN
       TE(1)=0.D0			!low Re (may be unnecessary, but just in case)
       TE(NY)=0.D0		!low Re (may be unnecessary, but just in case)
      ENDIF
